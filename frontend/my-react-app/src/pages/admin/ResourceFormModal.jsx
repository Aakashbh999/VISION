...
const ResourceFormModal = ({
  ...
  const handleFileChange = (e) => {
    ...
    // Validation (Limit to the configured backend size)
    if (selectedFile.size > MAX_FILE_SIZE_BYTES) {
      setFileError(
        `File size exceeds the ${MAX_FILE_SIZE_LABEL} limit. Please choose a smaller file.`,
      );
    ...
    if (!formData.title) {
      const prettyName = selectedFile.name
        .replace(/\.[^/.]+$/, "")
        .replace(/[_-]/g, " ");
  ...
  const handleAddCustomTag = (e) => {
    if (e.key === "Enter" || e.key === ",") {
      ...
      const systemTagMatch = allTags.find(
        (t) => t.name.toLowerCase() === cleaned,
      );
      if (systemTagMatch) {
        ...
        if (!selectedSystemTags.includes(systemTagMatch.tag_id)) {
          ...
          showToast.info(
            `System tag "${systemTagMatch.name}" is already selected`,
          );
        }
        setCustomTagInput("");
        return;
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.program_id)
      submissionData.append("program_id", formData.program_id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md overflow-y-auto">
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-border-main bg-gradient-to-r from-blue-500/5 to-purple-500/5">
          <div className="text-left">
            ...
            <p className="text-sm text-text-muted mt-1">
              {isEdit
                ? "Update configuration, classification details, or replace resource media."
                : "Add curriculum materials, notes, or books directly to the database."}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form
          ...
          className="p-6 md:p-8 space-y-6 max-h-[calc(85vh-8rem)] overflow-y-auto custom-scrollbar text-left"
        >
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="border-2 border-dashed border-border-main hover:border-blue-500/50 rounded-2xl p-5 flex flex-col items-center justify-center bg-bg-active/20 hover:bg-bg-active/40 transition-all cursor-pointer relative">
                      ...
                      <span className="text-xs text-text-muted mt-1">
                        PDF, DOCX, PPTX, ZIP or Images (Max{" "}
                        {MAX_FILE_SIZE_LABEL})
                      </span>
                    </div>
                    ...
                    {file && isFileTooLarge && (
                      <span className="text-xs text-red-500 flex items-center gap-1.5">
                        ...
                        Selected file is larger than {MAX_FILE_SIZE_LABEL}.
                      </span>
                    )}
                    ...
                    {isEdit && !file && resource?.file_url && (
                      <div className="text-xs text-text-muted flex items-center gap-2 bg-bg-active/50 p-2.5 rounded-xl border border-border-main truncate">
                        ...
                        <span className="truncate">
                          ...
                          >
                            {resource.original_filename || "View asset"}
                          </a>
                        </span>
                      </div>
                    )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-text-muted uppercase tracking-wider flex items-center gap-1.5">
                  <Tag className="w-3.5 h-3.5 text-blue-500" /> System
                  Categorization Tags
                </label>
            ...
            <div className="space-y-3">
              <label className="text-xs font-bold text-text-muted uppercase tracking-wider flex items-center gap-1.5">
                <Plus className="w-3.5 h-3.5 text-purple-500" /> Custom / User
                Tags
              </label>
              ...
              <p className="text-[10px] text-text-muted">
                Custom tags are verified by the system and converted
                automatically. Add multiple by splitting with comma.
              </p>
          ...
          <div className="bg-bg-active/30 p-5 rounded-2xl border border-border-main flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="text-left">
              <h4 className="text-sm font-bold text-text-main">
                Library Publication Settings
              </h4>
              <p className="text-xs text-text-muted mt-1">
                Configure content visibility. Setting status to Approved
                instantly publishes the content in library pools.
              </p>
            </div>

            <div className="flex items-center gap-3 w-full md:w-auto">
              <label className="text-xs font-bold text-text-muted uppercase shrink-0">
                Verification State
              </label>
              <select
                name="status"
