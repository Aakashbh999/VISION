...
import { getAdminDashboardStats, getModerationLogs } from "../../services/admin";
...
const StatCard = ({ title, value, icon: Icon, color, delay }) => {
  const colorMap = {
    ...
    yellow: "text-amber-500 bg-amber-500/10 border-amber-500/20 shadow-amber-500/10",
  ...
  return (
    ...
    >
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-transform group-hover:scale-110 ${colorMap[color]}`}>
        <Icon className="w-7 h-7" />
      </div>
      <div>
        <p className="text-sm font-bold text-text-muted uppercase tracking-wider">{title}</p>
        <p className="text-3xl font-black text-text-main mt-1 tracking-tight">{value}</p>
      </div>
    </motion.div>
  );
};
...
const ActivityItem = ({ log, index }) => {
  const getActionStyles = (action) => {
    switch (action) {
      ...
      case 'suspend': return 'bg-orange-500/10 text-orange-500 border-orange-500/20';
  ...
  return (
    ...
    >
      <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border whitespace-nowrap ${getActionStyles(log.action_type)}`}>
        {log.action_type}
      </div>
...
const Dashboard = () => {
  ...
  const totalUsers = (parseInt(users.approved) || 0) + (parseInt(users.pending) || 0) + (parseInt(users.rejected) || 0) + (parseInt(users.suspended) || 0);
  const logs = Array.isArray(logsData) ? logsData : (logsData?.data || []);

  const quickActions = [
    { label: "Verify Students", path: "/admin/pending", icon: Clock, color: "blue", count: users.pending },
    { label: "Pending Materials", path: "/admin/resources/pending", icon: FileText, color: "purple" },
    { label: "Reports Desk", path: "/admin/reports", icon: AlertTriangle, color: "red", count: stats.reports_open },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto pb-12 space-y-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="bg-bg-card border border-border-main px-6 py-3 rounded-2xl shadow-sm flex items-center gap-6">
          <div className="text-right">
            <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Server Status</p>
          ...
          <div className="text-right">
            <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Admin ID</p>
            <p className="text-sm font-black text-text-main tracking-tighter">#CMS-NODE-01</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard title="Total Users" value={totalUsers} icon={Users} color="blue" delay={0.1} />
        <StatCard title="Applications" value={users.pending || 0} icon={Clock} color="yellow" delay={0.2} />
        <StatCard title="Approved Students" value={users.approved || 0} icon={CheckCircle} color="green" delay={0.3} />
        <StatCard title="Rejected" value={users.rejected || 0} icon={XCircle} color="red" delay={0.4} />
        <StatCard title="Suspended" value={users.suspended || 0} icon={AlertTriangle} color="orange" delay={0.5} />
        <StatCard title="Active Reports" value={stats.reports_open || 0} icon={Activity} color="purple" delay={0.6} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Link to="/admin/logs" className="text-xs font-bold text-purple-600 hover:text-purple-700 transition-colors uppercase tracking-widest flex items-center gap-1">
            ...
            <div className="space-y-3">
              {logs.length === 0 ? (
                <div className="py-20 text-center space-y-3">
                  ...
                  <p className="text-text-muted font-medium">No recent moderation activity found.</p>
                </div>
              ) : (
                <>
                  {logs.map((log, i) => (
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-black text-text-main tracking-tight">Management Suite</h2>
          <div className="space-y-3">
            {quickActions.map((action, i) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <span className="text-sm font-bold text-text-main block">{action.label}</span>
                    {parseInt(action.count) > 0 && (
                      <span className="text-[10px] font-bold text-rose-500 uppercase">{action.count} ITEMS WAITING</span>
                    )}
                  </div>
                </div>
          ...
          <div className="p-6 bg-purple-500/5 border border-purple-500/20 rounded-3xl space-y-3">
             ...
             <p className="text-sm font-bold text-text-main">Access Notice</p>
             <p className="text-xs text-text-muted leading-relaxed font-medium">
                You are currently accessing the system with high-level administrative privileges. All actions are logged to the persistent audit trail.
             </p>
          </div>
        </div>
      </div>
