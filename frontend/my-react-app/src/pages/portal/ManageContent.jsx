...
const ManageContent = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      {/* Tab Content */}
      <div className="bg-[var(--bg-card)] rounded-3xl shadow-sm border border-[var(--border-main)] overflow-hidden min-h-[400px]">
        {activeTab === "resources" && (
          <div className="p-6 md:p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-black text-[var(--text-main)]">Resource Uploads</h2>
              <Link 
                to="/resources/my"
            ...
            {loadingResources ? (
              ...
              <div className="text-center py-20 bg-[var(--bg-active)] rounded-2xl border-2 border-dashed border-[var(--border-main)]">
                ...
                <h3 className="text-xl font-bold text-[var(--text-main)] mb-2">No Resources Found</h3>
                <p className="text-[var(--text-muted)] mb-6">You haven't uploaded any resources yet.</p>
                <Link to="/resources" className="px-6 py-2 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 transition-colors">
                  Go to Library
                </Link>
              </div>
            ) : (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-[var(--border-main)]">
                      <th className="pb-4 font-bold text-[var(--text-muted)] uppercase text-xs tracking-wider">Title</th>
                      <th className="pb-4 font-bold text-[var(--text-muted)] uppercase text-xs tracking-wider">Type</th>
                      <th className="pb-4 font-bold text-[var(--text-muted)] uppercase text-xs tracking-wider">Date</th>
                      <th className="pb-4 font-bold text-[var(--text-muted)] uppercase text-xs tracking-wider">Status</th>
                      <th className="pb-4 font-bold text-[var(--text-muted)] uppercase text-xs tracking-wider text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[var(--border-main)]">
                    {resources.map((resource) => (
                      <tr key={resource.resource_id} className="group hover:bg-[var(--bg-active)] transition-colors">
                        <td className="py-4 pr-4">
                          <p className="font-bold text-[var(--text-main)] mb-1 truncate max-w-xs">{resource.title}</p>
                        </td>
                        <td className="py-4 pr-4 text-[var(--text-muted)] capitalize text-sm">{resource.resource_type}</td>
        ...
        {activeTab === "discussions" && (
          <div className="p-6 md:p-8 text-center py-20">
            ...
            <h2 className="text-2xl font-black text-[var(--text-main)] mb-2">My Discussions</h2>
            <p className="text-[var(--text-muted)] mb-6">Manage your discussion threads here.</p>
        ...
        {activeTab === "groups" && (
          <div className="p-6 md:p-8 text-center py-20">
            ...
            <h2 className="text-2xl font-black text-[var(--text-main)] mb-2">My Groups</h2>
            <p className="text-[var(--text-muted)]">Manage groups you've created here. (Coming Soon)</p>
          </div>
        )}
      </div>
    </div>
