  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 px-2 sm:px-6 lg:px-8 py-5 sm:py-8 lg:py-10">
      ...
      <div className="min-h-[400px]">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-16 sm:py-24 bg-[var(--bg-card)]/50 border border-[var(--border-main)] border-x-0 sm:border-x rounded-sm sm:rounded-[2.5rem]">
            <LoadingSpinner />
          ...
          <div className="bg-rose-50 border border-rose-100 text-rose-600 p-8 sm:p-12 rounded-sm sm:rounded-[2.5rem] text-center font-bold">
            Failed to load roadmaps. Try again.
          </div>
        ) : roadmapsList.length === 0 ? (
          <div className="col-span-full bg-[var(--bg-card)] border border-dashed border-[var(--border-main)] rounded-sm sm:rounded-[2.5rem] p-8 sm:p-16 lg:p-24 text-center">
            {data?.noResults ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                  {data.recommendations?.roadmaps?.map((rec) => (
                    <Link
                      key={rec.id}
                      to={`/roadmaps/${rec.id}`}
                      className="group p-4 sm:p-5 bg-[var(--bg-active)] border border-transparent hover:border-purple-200 hover:bg-[var(--bg-card)] hover:shadow-xl rounded-sm sm:rounded-2xl transition-all duration-300 flex items-center gap-4"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8 mt-4">
            {roadmapsList.map((roadmap) => (
              <Link
                ...
                className="bg-[var(--bg-card)] rounded-sm sm:rounded-[2rem] border border-[var(--border-main)] border-x-0 sm:border-x p-5 sm:p-8 hover:shadow-2xl hover:border-purple-200 transition-all duration-300 group shadow-sm hover:-translate-y-2"
              >
                <div className="w-14 h-14 rounded-sm sm:rounded-2xl bg-[var(--bg-active)] flex items-center justify-center text-[var(--text-muted)] group-hover:bg-purple-600 group-hover:text-white transition-all mb-5 sm:mb-6">
                ...
                <p className="text-sm text-[var(--text-muted)] mb-5 sm:mb-6 line-clamp-2 font-medium leading-relaxed">
                  {roadmap.description}
                </p>
                <div className="flex items-center justify-between pt-4 sm:pt-6 border-t border-[var(--border-main)]">
