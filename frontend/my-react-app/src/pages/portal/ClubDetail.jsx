...
const ClubDetail = () => {
  ...
  if (error) return <div className="p-8 text-rose-500 font-bold text-center">Failed to load organization</div>;
  ...
  const socialLinks = [
    ...
    { key: "discord", icon: MessageCircle, label: "Discord", color: "text-indigo-600 bg-indigo-50 hover:bg-indigo-100", url: club.discord_url || parsedContact?.discord },
    { key: "github", icon: Github, label: "GitHub", color: "text-slate-800 bg-slate-100 hover:bg-slate-200", url: club.github_url || parsedContact?.github },
    { key: "reddit", icon: RedditIcon, label: "Reddit", color: "text-orange-600 bg-orange-50 hover:bg-orange-100", url: club.reddit_url || parsedContact?.reddit },
  ...
  return (
    ...
    >
      ...
      <OnboardingModal isOpen={isJoinModalOpen} onClose={() => setIsJoinModalOpen(false)} club={club} />
    </motion.div>
  );
};
