...
const DiscussionCard = ({ 
  ...
  return (
    <div
      className="flex bg-[var(--bg-card)] border border-[var(--border-main)] rounded-xl hover:border-purple-500/50 hover:bg-[var(--bg-active)] transition-all overflow-hidden group shadow-sm"
    >
      {/* Voting Rail */}
      <div className="w-12 bg-[var(--bg-main)]/30 flex flex-col items-center py-4 gap-2 border-r border-[var(--border-main)]/50">
        <button
      ...
      <div className="flex-1 p-4 flex flex-col">
        ...
        <Link to={`/discussions/${disc.discussion_id}`} className="block group/title">
          <h2 className="text-lg font-black text-[var(--text-main)] leading-tight mb-2 group-hover/title:text-purple-500 transition-colors">
            {disc.title}
          </h2>
                ...
                <AnimatePresence>
                  {isImageLoading && (
                    <motion.div 
                      initial={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="absolute inset-0 z-10 bg-[var(--bg-card)] flex items-center justify-center"
                    >
