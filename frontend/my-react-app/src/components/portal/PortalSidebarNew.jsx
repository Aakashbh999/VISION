...
const NavItem = ({ item, isCollapsed, depth = 0 }) => {
  ...
  const linkClasses = `
    ...
    ${
      isActive
        ? "bg-sidebar-active-bg text-purple-700 dark:text-purple-400 font-medium border-l-[3px] border-purple-500"
        : "text-text-muted hover:bg-sidebar-hover-bg hover:text-purple-600 dark:hover:text-purple-400 border-l-[3px] border-transparent"
    }
...
const PortalSidebarNew = () => {
    ...
    <div className="flex flex-col h-full">
      ...
      {!isMobile && (
        <button
          ...
          className="absolute -right-3 top-20 w-6 h-6 bg-bg-card border border-border-main rounded-full flex items-center justify-center shadow-sm hover:bg-sidebar-hover-bg hover:border-purple-200 transition-colors z-50"
      ...
      {(!isCollapsed || isMobile) && (
        <div className="p-3 border-t border-border-main">
          <div className="p-3 rounded-lg bg-sidebar-active-bg border border-blue-100 dark:border-blue-900/30">
            <p className="text-xs text-text-muted font-medium">Learning Streak</p>
        ...
        {isMobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 h-full w-64 bg-bg-main border-r border-border-main shadow-xl z-50 lg:hidden"
            >
              {sidebarContent}
            </motion.aside>
          </>
  ...
  return (
    <motion.aside
      ...
      className="hidden lg:block fixed left-0 top-16 h-[calc(100vh-4rem)] bg-bg-main/95 backdrop-blur-sm border-r border-border-main shadow-sm z-30 transition-colors duration-300"
    >
      {sidebarContent}
    </motion.aside>
  );
