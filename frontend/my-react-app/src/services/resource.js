...
export const getResources = async (filters = {}) => {
  ...
  if (view === "my") {
    const response = await api.get("/api/resources/my");
  ...
  const response = await api.get("/api/resources", { params: otherFilters });
  return response.data;
};
...
export const softDeleteResource = async (resourceId, reason) => {
  const response = await api.post(`/api/resources/${resourceId}/soft-delete`, {
    reason,
  });
  return response.data;
};
