...
export const getRoadmaps = async (filters = {}) => {
  const response = await api.get("/api/roadmaps", { params: filters });
...
export const getStepResources = async (id, stepId) => {
  const response = await api.get(
    `/api/roadmaps/${id}/steps/${stepId}/resources`,
  );
  return response.data;
};
...
export const completeStep = async (stepId) => {
  const response = await api.post(`/api/roadmaps/steps/${stepId}/complete`);
  return response.data;
};
