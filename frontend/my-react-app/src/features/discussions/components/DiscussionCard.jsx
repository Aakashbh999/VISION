...
const DiscussionCard = memo(
  ({
    ...
    const authorImage =
      ...
      null;
    const isAuthor =
      user && String(user.portal_user_id) === String(authorProfileId);
    ...
    const likeCount = Math.max(
      ...
      baseLikeCount +
        (baseVote === 1 ? -1 : 0) +
        (optimisticVote === 1 ? 1 : 0),
    );
    const isLiked = optimisticVote === 1;
    const isSaved = optimisticSaved;
    const isBoosted =
      disc.is_boosted && new Date(disc.boosted_until) > new Date();
        ...
        <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-700 flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          >
            {loadingLike === disc.discussion_id ? (
              ...
              <ThumbsUp
                className={`w-4 h-4 pointer-events-none ${isLiked ? "fill-current" : ""}`}
              />
            )}
          ...
          >
            {loadingSave === disc.discussion_id ? (
              ...
              <Bookmark
                className={`w-4 h-4 pointer-events-none ${isSaved ? "fill-current" : ""}`}
              />
            )}
            Save
          </button>
