 ...
 * Manages learning roadmap CRUD and hierarchical step structure.
 * Allows content creators to build and maintain learning paths.
 *
 * Features:
 * - Roadmap creation and metadata management
 * - Roadmap retrieval with step counts and structure
 * - Roadmap updates and soft deletion
 * - Step addition/removal/reordering within roadmaps
 * - Resource linking to roadmap steps
 * - Roadmap ordering (drag-and-drop support)
 * - Activity logging for all roadmap changes
 */
...
exports.createRoadmap = catchAsync(async (req, res) => {
  ...
  const existing = await pool.query(
    `SELECT 1 FROM portal.roadmaps WHERE slug = $1`,
    [slug],
  );
...
exports.updateRoadmap = catchAsync(async (req, res) => {
  ...
  const existing = await pool.query(
    `SELECT 1 FROM portal.roadmaps WHERE slug = $1 AND roadmap_id != $2`,
    [slug, id],
  );
  if (existing.rows.length > 0) {
    slug = `${slug}-${Date.now()}`;
...
exports.reorderStep = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const currRes = await client.query(
      `SELECT roadmap_id, step_order FROM portal.roadmap_steps WHERE step_id = $1 AND deleted_at IS NULL`,
      [stepId],
    );
    ...
    await client.query(
      `UPDATE portal.roadmap_steps SET step_order = -1 WHERE step_id = $1`,
      [stepId],
    );
    await client.query(
      `UPDATE portal.roadmap_steps SET step_order = $1 WHERE step_id = $2`,
      [currOrder, adjId],
    );
    await client.query(
      `UPDATE portal.roadmap_steps SET step_order = $1 WHERE step_id = $2`,
